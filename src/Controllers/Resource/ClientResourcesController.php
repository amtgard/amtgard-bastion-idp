...
class ClientResourcesController
{
    ...
    private function requireUser(mixed $idpUserId, Response $response): UserEntity|Response
    {
        ...
        if ($publicId === '') {
            return $this->jsonError($response, self::ERROR_IDP_USER_ID_REQUIRED, 400);
        }

        return $this->requestResolver->findUserByPublicId($publicId)
            ->orElseGet(fn () => $this->jsonError($response, self::ERROR_UNKNOWN_IDP_USER_ID, 404));
    }
    ...
    private function requireLoginForUser(mixed $loginId, UserEntity $user, Response $response): array|Response
    {
        if (!is_numeric($loginId) || (int) $loginId <= 0) {
            return $this->jsonError($response, self::ERROR_LOGIN_ID_REQUIRED, 400);
        }

        return $this->requestResolver->findLoginIdForUser((int) $loginId, $user->getId())
            ->map(fn (int $resolvedLoginId) => ['user' => $user, 'loginId' => $resolvedLoginId])
            ->orElseGet(fn () => $this->jsonError($response, self::ERROR_UNKNOWN_LOGIN_ID, 404));
    }

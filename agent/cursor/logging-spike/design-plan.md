...
4. **Logging never risks operations (non-negotiable):** no request, OAuth step, PVH gate, or repository call may fail, slow materially, or roll back because logging failed. Handlers are wrapped in `WhatFailureGroupHandler` (or equivalent) so I/O exceptions are swallowed after optional stderr counter. **SQLite and any durable sink are strictly off the hot path** (see async persistence below).
5. **One logger seam in constructors:** application code injects a single **`IdpLogging`** (name TBD) coalescing facade — not five `LoggerInterface` parameters. Channel selection is a method call, not a DI type hint.
...
### DI binding pattern — coalescing container (stakeholder requirement)

**Problem:** multiple Monolog channels must not multiply constructor parameters (`$appLogger`, `$securityLogger`, …).

**Pattern:**

- Register **one** application type, e.g. **`IdpLogging`**, in PHP-DI. It owns channel construction, processor stacks, and handler wiring internally (backed by `LogChannelFactory`).
- Call sites use channel selectors on that single dependency:

```php
// Conceptual API — exact naming at implementation time
$idpLog->app()->info('…', $ctx);
$idpLog->security()->error('…', $ctx);
$idpLog->forChannel(LogChannel::Oauth)->debug('…', $ctx);
```

- **`LoggerInterface` binding (legacy):** delegate to `$idpLog->app()` so existing injections keep working during migration; new code should prefer `IdpLogging` only.
- **Forbidden in new code:** injecting more than one logger-typed parameter per class (enforced by convention + optional PHPStan rule in rollout).
- Workers receive the same `IdpLogging` instance; set **`job_id`** + inherited **`request_id`** when processing queue messages that originated from HTTP.
...
- **Async durable sink** → hourly SQLite (see writer behavior) — never attached directly to the request thread.
- Optional plain-text JSONL mirror (staging/spike).

**Error stack** (levels ERROR → EMERGENCY, 30-day retention):

- Same async SQLite pipeline, separate hourly files / stream column.
- Duplicate ERROR+ into trace stream within retention window (same `request_id` joins).

**Live ops tail (stakeholder preference):** keep a **synchronous, low-latency `php://stderr` JSONL handler** (or existing `ErrorLogHandler` when stderr is the PHP error log target) on both trace and error stacks, **above** the async queue, with minimal formatting and the same processor stack. Operators can `tail -f` container stderr or local dev output without waiting on SQLite or the log-read API. The polling API / CLI remain the structured query path; stderr is the “what is happening right now” path.
...
### Writer behavior — async, non-blocking, WAL (stakeholder requirement)

**Hot path (HTTP worker, PVH worker, CLI serving traffic):**

1. Build record (processors: correlation, redaction).
2. Write to **stderr JSONL** synchronously (small buffer; failures ignored).
3. Enqueue a **serialized copy** for durable storage via **`AsyncLogQueueHandler`** (Monolog handler that only pushes to an in-memory ring buffer or spool — no SQLite in this call stack).
4. Return immediately. **No `open()`/`INSERT`/`close()` on SQLite in the request process.**

If the queue is full: **drop durable enqueue** (increment `log_dropped_total`, emit one stderr warning per interval), never block or throw into application code.

**Background persistence (logging subsystem only):**

- A dedicated **`bin/log-sqlite-writer.php`** (or equivalent long-lived sidecar under systemd/supervisor) drains the queue/spool and is the **only** component that opens SQLite files.
- Per file: `PRAGMA journal_mode=WAL;`, `PRAGMA synchronous=NORMAL;` (spike: measure; tighten only if corruption tests pass).
- **Open → batch insert → close** per flush cycle (or keep one connection per hourly file in the writer process only). Target: sub-millisecond hot path, millisecond-scale batched writes off-thread.
- Batch insert in a transaction every N records or ~100ms (whichever comes first).
- Hour rotation: writer closes current handle and opens the new hourly path at UTC boundary — rotation must not run inside FPM request shutdown.

**Spool fallback (recommended for multi-worker PHP-FPM):** if in-process ring buffer is insufficient, hot path appends **one line per record** to `LOG_ROOT/spool/*.jsonl` with `LOCK_EX` (fast append-only); writer consumes and deletes spool segments. Same non-blocking contract.

**Do not** store SQLite or spool under Google Drive sync path in dev — use `LOG_ROOT` outside sync (see risks).

```mermaid
flowchart LR
  APP[Application IdpLogging]
  PROC[Processors]
  STDERR[stderr JSONL sync]
  Q[Async queue or spool]
  WR[log-sqlite-writer process]
  SQL[(SQLite hourly WAL)]
...
2. **`LogReaderAuthorizationMiddleware`:** resolves user, loads policy, checks ORN requirement for route. **`IDP/AnalyzeLogs` satisfies any log-read route** (trace, error, stats, analyze) so CLI-hoisted agent JWTs need only one claim.
...
| `Idp:0::::IDP/AnalyzeLogs` | Desktop agents, review bots, **small resolution team** | **Coarse grant:** trace + error search, request detail, stats, and bounded analyze export (no `--redact-off`). Satisfies all log-read API routes when finer `Read*` claims are not issued separately. |
...
- **Desktop / IDE agents (Cursor, review bots):** often **no IDP username/password** and no interactive OAuth. Use a **operator-issued long-lived JWT** (see below), not a human session.

Register claims in `orn-definitions` / `IdpClaim` resource map in implementation phase; spike only documents strings.

### Agent log-read JWT — CLI hoist to local `.env` (stakeholder requirement)

Agents need to call the log-read API (`GET /resources/logs/…`, `POST /resources/logs/analyze`) with a normal **`Authorization: Bearer`** JWT carrying **only** **`Idp:0::::IDP/AnalyzeLogs`**. Log resolution is infrequent and the team is tiny (operator + desktop agent), so **one coarse ORN** is intentional — no separate `ReadTraceLogs` / `ReadErrorLogs` on the CLI token. They should not depend on browser login or password grants.

**Tool (design):** `bin/idp-agent-log-token` (exact name TBD)

| Subcommand | Purpose |
|------------|---------|
| `issue` | Mint a signed IDP JWT for a fixed **agent subject** (dedicated user UUID or service identity) with **`AnalyzeLogs` only** |
| `refresh` | Re-issue before expiry using the same local operator credential |
| `print-env` | Emit a single line suitable for `.env`: `IDP_LOG_READ_JWT=eyJ…` (no other secrets on stdout) |

**Operator auth for CLI (pick one in implementation; document clearly):**

- Local **management key** + `--host` (HTTPS to staging/prod), or
- Run on host with access to **`OAUTH_*` signing material** (same as IDP mint path) so the CLI signs offline without storing passwords.

**JWT profile:**

- Claims: `sub`, `aud` (log-reader client id), `policy` / ORN claim **`AnalyzeLogs` only**, `iat`, `exp`.
- TTL: **long-lived but bounded** (e.g. 30–90 days default; configurable). Not refresh tokens in v1 — operator re-runs `issue` or `refresh`.
- **No** `EditClient`, **no** broad admin policy.

**Exfiltration to the agent (explicitly local, operator-controlled):**

1. Operator runs `bin/idp-agent-log-token print-env >> ~/.config/amtgard-idp/agent.env` (or project-local `.env.agent`, gitignored).
2. Cursor / local tooling loads `IDP_LOG_READ_JWT` and `IDP_BASE_URL` when calling the log-read API or a thin wrapper script.
3. Token never committed; `.env*` patterns already excluded from git.

**Security notes:**

- Treat issued JWT like a **secret** (equivalent to a PAT). Revoke by removing IAM claims for that `sub` or rotating signing keys.
- Rate limits on log-read API still apply per `sub`.
- Optional future: bind JWT to `client_id` + mTLS for prod; spike stays Bearer + ORN.

**Relation to OAuth client credentials:** server-side automation can still use client credentials; the CLI path is for **human-operated desktops** where installing a confidential client is awkward but pasting one env var is not.
...
- [ ] `LOG_ROOT`, `IdpLogging` coalescing facade + correlation middleware + processors (correlation + redaction)
- [ ] stderr JSONL handler + async queue/spool + `bin/log-sqlite-writer.php` (WAL hourly files)
- [ ] Prove hot-path logging does not increase request latency beyond stderr write (SQLite offline)
- [ ] `HttpObservabilityMiddleware` on `/resources/validate` + one OAuth route
- [ ] CLI `idp-logs query --request-id`
- [ ] Document env vars and denylist
- **Exit criteria:** Reconstruct one staged request end-to-end from SQLite using only `request_id`; kill sqlite-writer mid-traffic and confirm HTTP still succeeds
...
- [ ] `AnalyzeLogs` for agent workflows; `bin/idp-agent-log-token` + `.env` hoist doc; review doc updates
...
| **SQLite write latency** | Request tail latency | Async queue/spool + dedicated writer; WAL; never SQLite on hot path |
| **Queue/spool full** | Lost durable logs | Drop + metric; stderr still works; never fail requests |
...
11. **Agent JWT TTL default:** 30 vs 90 days for `idp-agent-log-token issue`?
12. **FPM spool vs in-memory queue:** mandatory spool dir when `pm.max_children` > 1?
...
| `IdpLogging` | Single DI facade; channel selection for all call sites |
| `LogChannelFactory` | Channel construction (used only by `IdpLogging`) |
| `AsyncLogQueueHandler` | Hot-path enqueue; no SQLite |
| `bin/log-sqlite-writer.php` | Drain queue/spool → SQLite WAL |
| `LogReaderAuthorizationMiddleware` | ORN checks |
| `LogsQueryService` | SQL builder, caps, cursors |
| `bin/idp-logs` | Operator CLI |
| `bin/idp-agent-log-token` | Mint long-lived log-read JWT → `.env` hoist |
...
| `LOG_STDERR_ENABLED` | `true` | Sync stderr JSONL tail |
| `LOG_ASYNC_QUEUE_SIZE` | `10000` | In-memory ring; overflow → drop durable |
| `LOG_SPOOL_ENABLED` | `true` prod FPM | Append-only spool before sqlite writer |
| `IDP_LOG_READ_JWT` | (unset) | Local agent `.env`; not server env |

---

## Stakeholder feedback (2026-09-11)

Captured in this revision:

| Topic | Decision |
|-------|----------|
| Multiple loggers in constructors | **No** — single `IdpLogging` coalescing container |
| SQLite / durable I/O | **Async only**; WAL in writer process; open/write/close fast; never block operations |
| Operational risk | Logging **must not** fail or endanger application work (`WhatFailureGroupHandler` + drop-on-full queue) |
| stderr tail | **Keep** sync stderr JSONL for live tail; API/CLI for structured poll/search |
| Agents without IDP login | **`bin/idp-agent-log-token`** → long-lived JWT with **`AnalyzeLogs` only** in local `.env` |
| Agent IAM granularity | **Single coarse ORN** (`AnalyzeLogs`); fine-grained `Read*` for humans/support only |
